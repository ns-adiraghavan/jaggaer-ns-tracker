...
// ─── Brand detection ────────────────────────────────────────────────────────
// A query is "branded" when it names Jaggaer (incl. common misspellings and the
// site: operator). Branded / navigational queries rank far better than the
// organic average, so we split them out: the NON-branded number is the honest
// "how do we rank for the terms we're actually trying to win" figure. Keep this
// list in sync with perfIsBranded() in performance.jsx.
...
// Impression-weighted position over a query list, split branded vs non-branded.
// Weighted position = Σ(impressions × position) / Σ(impressions), taken over
// EVERY query (never a top-N slice — truncating skews the number).
function queryAggregates(queries) {
  let wImpr = 0, wSum = 0;
  const mk = () => ({ impressions: 0, clicks: 0, wImpr: 0, wSum: 0, query_count: 0 });
  const b = mk(), n = mk();
  for (const q of queries) {
    const bucket = isBranded(q.query) ? b : n;
    bucket.impressions += q.impressions || 0;
    bucket.clicks += q.clicks || 0;
    bucket.query_count += 1;
    if (q.impressions > 0 && q.position > 0) {
      wImpr += q.impressions; wSum += q.impressions * q.position;
  ...
  const pack = x => ({
    ...
    position_impressions: x.wImpr,   // denominator, for portfolio rollups
  });
  return {
    query_weighted_position: wImpr > 0 ? wSum / wImpr : 0,
    query_position_impressions: wImpr,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Query-level aggregates — over the FULL query list, branded split out ──
    const allQueries = dimRows(queryRows, "query");
    const agg = queryAggregates(allQueries);
    ...
    const summary = {
      ...
      avg_position,                                        // GSC page-level position
      query_weighted_position: agg.query_weighted_position, // Σ(impr×pos)/Σimpr, ALL queries
      query_position_impressions: agg.query_position_impressions,
      query_count: agg.query_count,
      branded: agg.branded,          // { impressions, clicks, weighted_position, query_count, ... }
      nonbranded: agg.nonbranded,    // same shape — the honest organic figures
    };
    ...
    return res.status(200).json({
      ...
      summary,
      timeseries,
      // Store the full query list (capped at 200 for size) tagged branded/not,
      // so the UI can show top-15 with expand-to-all and flag branded rows.
      top_queries: allQueries.slice(0, 200).map(q => ({ ...q, branded: isBranded(q.query) })),
