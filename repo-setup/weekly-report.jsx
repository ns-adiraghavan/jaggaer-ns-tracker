...
function WeeklyReportPanel({ project, currentUser }) {
  ...
  return (
    <main className="ns-tracker" style={{ padding: "28px 32px", maxWidth: "1100px" }}>
      ...
      <Section title="Took Live This Week" count={data.tookLive.length} accent="#1e7a45">
        {data.tookLive.length === 0 ? (
          <EmptyRow text="Nothing moved to Approved in the last 7 days." />
          ...
          <ReportRow key={piece.id}
            ...
            right={ts ? `${formatEST(ts, { hour: "2-digit", minute: "2-digit" })} EST` : "date not logged"}
            rightSub={approvedBy ? memberLookup(project)(approvedBy)?.name : null}
          />
        ))}
      </Section>

      <Section title="Plan to Take Live Next Week" count={data.planNextWeek.length} accent="#1e6fa8"
        subtitle="Past Abhishek + Orlagh review — sitting at CTA check, Robert review, or Ad-Hoc review.">
        {data.planNextWeek.length === 0 ? (
          <EmptyRow text="No pieces currently past marketing review." />
      ...
      <div style={{ height: "1px", background: "#e8e3da", margin: "32px 0" }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "8px" }}>
        <Section title="Pending at Jaggaer" count={data.pendingAtJaggaer.length} accent="#c8401a" compact>
          {data.pendingAtJaggaer.length === 0 ? (
            <EmptyRow text="Nothing waiting on Jaggaer right now." />
        ...
        <Section title="Pending at NS" count={data.pendingAtNS.length} accent="#1e6fa8" compact>
          {data.pendingAtNS.length === 0 ? (
            <EmptyRow text="Nothing waiting on NS right now." />
      ...
      <Section title="SEO Briefs Not Yet Shared" count={data.briefsOutstanding.length} accent="#b05e00"
        subtitle="Pieces with no brief/keyword file attached yet (excludes Ad-Hoc Articles, which skip the brief flow).">
        {data.briefsOutstanding.length === 0 ? (
          <EmptyRow text="Every in-flight piece has a brief attached." />
      ...
      <Section title="Upload & Brief Log" count={data.uploadLog.length} accent="#7d6608"
        subtitle="Every NS deliverable upload and every Jaggaer brief/keyword file, newest first — use this to see the cadence and spot gaps.">
        {data.uploadLog.length === 0 ? (
          <EmptyRow text="No logged uploads yet." />
          ...
          <ReportRow key={i} compact
            ...
            rightSub={entry.by ? memberLookup(project)(entry.by)?.name : null}
          />
        ))}
        {data.uploadLog.length > 40 && (
          <div style={{ ...FONT, fontSize: "0.72rem", color: "#aaa", padding: "8px 4px" }}>
...
function Section({ title, count, accent, subtitle, compact, children }) {
  const FONT = { fontFamily: "Noto Sans, sans-serif" };
  return (
    <div style={{ marginBottom: compact ? "0" : "28px" }}>
      <div style={{
        display: "flex", alignItems: "baseline", gap: "8px",
        borderLeft: `3px solid ${accent}`, paddingLeft: "10px", marginBottom: "4px",
      }}>
      ...
      {subtitle && (
        <div style={{ ...FONT, fontSize: "0.72rem", color: "#999", paddingLeft: "13px", marginBottom: "10px", lineHeight: 1.4 }}>
          {subtitle}
        </div>
      )}
      <div style={{
        background: "#fff", border: "1px solid #e8e3da", borderRadius: "4px",
        overflow: "hidden", marginTop: subtitle ? 0 : "8px",
      }}>
        {children}
      </div>
    </div>
...
function EmptyRow({ text }) {
  return (
    <div style={{ fontFamily: "Noto Sans, sans-serif", fontSize: "0.76rem", color: "#aaa", padding: "16px 14px" }}>
      {text}
    </div>
  );
}
