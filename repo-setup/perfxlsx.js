...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Query-level weighted position — over the FULL query list ──────────
    // The stored top_queries is capped at 25 for size, but the impression-
    // weighted average position must be computed across EVERY query or it skews
    // high (the long tail sits at good positions and pulls the number down).
    // Σ(query impressions × query position) / Σ(query impressions).
    const allQueries = dimRows(queryRows, "query");
    let qwImpr = 0, qwSum = 0;
    for (const q of allQueries) {
      if (q.impressions > 0 && q.position > 0) { qwImpr += q.impressions; qwSum += q.impressions * q.position; }
    }
    const query_weighted_position = qwImpr > 0 ? qwSum / qwImpr : 0;
    ...
    const summary = {
      ...
      avg_position,                         // GSC page-level position (kept)
      query_weighted_position,              // weighted across ALL queries
      query_position_impressions: qwImpr,   // denominator, for portfolio rollups
    ...
    return res.status(200).json({
      ...
      summary,
      timeseries,
      top_queries: allQueries.slice(0, 25),
